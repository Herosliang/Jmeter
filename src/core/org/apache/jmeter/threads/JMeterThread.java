...
import com.ofo.test.bo.Saveservices;
...
public class JMeterThread implements Runnable, Interruptible {
    ...
    //手机测试结果存储到myqsl数据库
    List<SampleEvent> listSampleEvent = new ArrayList<SampleEvent>();
    @Override
    public void run() {
        ...
        try {
            ...
            //引入Mojo，返回listSampleEvent，MOjo接受数据存储
            addTomysql(listSampleEvent);
        }
    ...
    //添加操作
    private void addTomysql(List<SampleEvent> list){

        int totalTest = list.size();
        int successTest = 0 ;
        int failedTest = 0;
        String label = "";
        int isSuccessful = 1;
        String jmx_name = "";
        for (SampleEvent se:list){
            if((se.getResult().isSuccessful())){
                successTest++;
            }else {
                failedTest++;
            }
        }
        label = list.get(0).getResult().getSampleLabel();
        ...
        //jmx_name = (String)this.engine.test.get(0);
        TestPlan testPlan = (TestPlan)this.engine.test.list().iterator().next();
        jmx_name = testPlan.getName();
        JSONObject obj = new JSONObject();
        obj.put("totalTest", totalTest);
        obj.put("successTest", successTest);
        obj.put("failedTest", failedTest);
        obj.put("label", label);
        obj.put("isSuccessful",isSuccessful);
        obj.put("create_at",new Timestamp(System.currentTimeMillis()));
        obj.put("jmx_name",jmx_name);
        Saveservices.resulteSave(obj);
    }
    ...
    private void notifyListeners(List<SampleListener> listeners, SampleResult result) {
        ...
        //把测试结果添加到list，一次返回到Mojo进行存储数据库
        listSampleEvent.add(event);
